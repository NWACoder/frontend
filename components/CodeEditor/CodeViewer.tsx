import React, { MouseEventHandler, useEffect, useReducer } from 'react';
import { Item } from '../../types';
import { init, initialState, reducer } from '../../lib/reducer/editor';
import { ActionType, EditorMode, Snippet } from '../../types';
import { Editor } from './Editor';

interface CodeViewer {
    snippet?: Snippet;
}

export const CodeViewer = ({ snippet: _snippet }: CodeViewer) => {
    const [state, dispatch] = useReducer(reducer, initialState(), init);

    useEffect(() => {
        if (_snippet) {
            dispatch({ type: ActionType.INIT_SNIPPET, payload: _snippet });
        }
    }, [_snippet]);

    const handleSelectItem: MouseEventHandler<HTMLButtonElement> = ({
        currentTarget,
    }) => {
        const id = currentTarget.getAttribute('data-id');
        dispatch({
            type: ActionType.SELECT_ITEM,
            payload: id,
        });
    };

    const handleChangeItemContent = ({
        value = '',
        id,
    }: {
        value: string | undefined;
        id: string;
    }) => {
        dispatch({
            type: ActionType.UPDATE_ITEM_CONTENT,
            payload: { value, id },
        });
    };

    const handleUpdateEditorMode: React.MouseEventHandler<HTMLButtonElement> =
        ({ currentTarget }) => {
            const mode = currentTarget.getAttribute('data-mode');
            if (!mode) return;
            dispatch({ type: ActionType.UPDATE_EDITOR_MODE, payload: mode });
        };

    const { snippet, selectedItemID, isMarkDown, mode } = state;
    const selectedItem = snippet.items.find(
        (item) => item.id === selectedItemID
    );
    return (
        <form className="flex flex-col flex-grow items-center w-full text-center">
            <div className="flex flex-row my-4 items-center md:w-full md:max-w-4xl flex-wrap-reverse justify-center">
                <div className="md:mr-auto">
                    <div className="form-group relative mt-4 md:mt-0">
                        <input
                            id="snippetName"
                            name="snippetName"
                            type="text"
                            className="border-dashed border-2 border-gray-400 text-xl focus:outline-none focus:ring-0"
                            value={snippet.title}
                            disabled
                        />
                        <label
                            className="text-base font-semibold text-left ml-1 opacity-100 px-2 floating-label-view"
                            aria-labelledby="snippetName"
                        >
                            Snippet Name
                        </label>
                        <div className="text-red-700 font-semibold">
                            {/* {errors.email?.message} */}
                        </div>
                    </div>
                </div>
                <div>
                    <div className="flex flex-row mb-4">
                        <DetailRectangle
                            value={snippet.items.length}
                            name="file"
                        />
                        <DetailRectangle value={0} name="fork" />
                        <DetailRectangle value={0} name="star" />
                    </div>
                    {/* TODO Put tags here */}
                </div>
            </div>
            <div className="flex flex-row py-16 max-w-max mx-auto bg-gray-200 h-full px-8">
                <div>
                    <CodeEditorNav
                        preview={isMarkDown}
                        mode={mode}
                        onClick={handleUpdateEditorMode}
                    />
                    <Editor
                        item={selectedItem}
                        handleChangeItemContent={handleChangeItemContent}
                        mode={mode}
                        readOnly={true}
                    />
                </div>
                <div className="flex flex-col ml-4">
                    <ItemListHeader title="Files" />
                    <SnippetFiles
                        items={snippet.items}
                        selectedItemID={selectedItemID}
                        onClickItem={handleSelectItem}
                    />
                </div>
            </div>
        </form>
    );
};

interface ItemListHeader {
    title: string;
    onClick?: () => void;
}

const ItemListHeader = ({ title }: ItemListHeader) => {
    return (
        <div className="w-52 flex flex-row items-end h-12 place-self-end pb-1">
            <div className="mr-auto">{title}</div>
        </div>
    );
};

interface DetailRectangle {
    value: number;
    name: string;
}

const DetailRectangle = ({ value, name }: DetailRectangle) => {
    return (
        <div className="bg-gray-200 rounded py-2 mx-2 w-24 whitespace-nowrap">
            {value === 1 ? `${value} ${name}` : `${value} ${name}s`}
        </div>
    );
};

interface CodeEditorNav {
    preview: boolean;
    mode: EditorMode;
    onClick: React.MouseEventHandler;
}

const CodeEditorNav = ({ preview, mode, onClick }: CodeEditorNav) => {
    return (
        <div className="flex flex-row mr-auto">
            <button
                className={`w-28 py-3 mr-4 hover:bg-gray-300 hover:text-black font-semibold ${
                    mode === 'editor'
                        ? 'bg-gray-300'
                        : 'bg-gray-200  border-2 border-b-0 border-gray-300 text-gray-600'
                }`}
                type="button"
                data-mode="editor"
                onClick={onClick}
            >
                Editor
            </button>
            {preview && (
                <button
                    className={`w-28 py-3 hover:bg-gray-300 hover:text-black font-semibold ${
                        mode === 'preview'
                            ? 'bg-gray-300'
                            : 'bg-gray-200  border-2 border-b-0 border-gray-300 text-gray-600'
                    }`}
                    type="button"
                    data-mode="preview"
                    onClick={onClick}
                >
                    Preview
                </button>
            )}
        </div>
    );
};

interface SnippetFiles {
    items: Item[];
    selectedItemID: string | null;
    onClickItem: React.MouseEventHandler;
}

export const SnippetFiles = ({
    items = [],
    selectedItemID,
    onClickItem,
}: SnippetFiles) => {
    return (
        <div className="border-2 border-gray-300 w-52 bg-white p-2 flex-1">
            {items.map((item) => {
                return (
                    <ItemButton
                        key={item._id}
                        item={item}
                        onClick={onClickItem}
                        selected={item.id === selectedItemID}
                    />
                );
            })}
        </div>
    );
};

interface ItemButton {
    item: Item;
    selected: boolean;
    onClick: React.MouseEventHandler;
}

const ItemButton = ({ item, selected, onClick }: ItemButton) => {
    const { name, id } = item;
    return (
        <div className="flex flex-row h-8 items-center hover:bg-gray-300">
            <button
                className={`px-2 py-1 w-full text-left ${
                    selected && 'bg-gray-200'
                }`}
                type="button"
                onClick={onClick}
                data-id={id}
            >
                <span className="flex-1">{name}</span>
            </button>
        </div>
    );
};
